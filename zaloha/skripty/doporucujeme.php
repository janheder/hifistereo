...
if(!$limit)
 ...
 $sql_dph = 'id_dph';
	...
	$query_pocet = MySQL_Query("SELECT count(P.id) AS CID FROM produkty P WHERE P.aktivni=1 AND P.doporucujeme=1") or die(err(1));
	...
	$query_n = MySQL_Query("SELECT P.id, P.str, P.nazev, P.popis, P.$cena_db as CENA_DB, P.cena_imaginarni, P.sleva, P.vyrobce,
		...
		WHERE P.aktivni=1  AND P.doporucujeme=1 AND O.typ=1 GROUP BY P.id  ORDER BY P.datum DESC LIMIT $limit, ".intval($_SESSION['pocet_strankovani'])." ") or die(err(1));
 ...
 $Sablonka->PridejDoSablonky('[nadpis]','Doporučujeme','html');
